  ...
  return (
    <div className="rounded-sheet border border-hairline bg-surface p-4 shadow-rest">
      ...
      <div className="flex h-9 w-full overflow-hidden rounded-control bg-track" role="presentation">
        ...
        >
          <span className="px-1 text-[9px] font-bold text-white">BMR</span>
        </div>
        <div
          className="flex flex-1 items-center justify-center"
          style={{ background: 'color-mix(in srgb, var(--brand) 35%, transparent)' }}
        >
          <span className="px-1 text-[9px] font-bold text-brand-ink">Activity</span>
        </div>
      </div>
      ...
      <dl className="mt-4 space-y-2.5">
        ...
        <div className="border-t border-hairline pt-2.5">
          <div className="flex items-baseline justify-between gap-3">
            <dt className="text-[13px] font-bold text-ink">Maintenance</dt>
            <dd className="font-display text-[20px] font-bold tabular-nums text-ink">
              {tdee.toLocaleString('en-IN')}
              <span className="ml-1 text-[11px] font-medium text-ink-3">kcal</span>
            </dd>
          </div>
  ...
  return (
    <div className="flex items-baseline justify-between gap-3">
      <div className="min-w-0">
        ...
        <p className="mt-0.5 text-[10.5px] leading-snug text-ink-3">{hint}</p>
      </div>
      <dd className="shrink-0 text-[15px] font-bold tabular-nums text-ink">
        {prefix}{value.toLocaleString('en-IN')}
        <span className="ml-1 text-[10px] font-medium text-ink-3">kcal</span>
      </dd>
    </div>
  )
}
