    ...
    <div className="rounded-card overflow-hidden bg-surface shadow-rest">
      ...
      <div className="flex items-center justify-between px-4 py-[14px]">
        <div className="flex items-center gap-3">
          <div className="flex h-[34px] w-[34px] flex-shrink-0 items-center justify-center rounded-control bg-surface-2 text-caption font-semibold text-ink-2">
            {cfg.letter}
          </div>
          <div>
            <p className="text-body font-semibold text-ink leading-none">{cfg.label}</p>
            {lastLogged && (
              <p className="text-caption text-ink-3 mt-[3px]">{formatTime(lastLogged)}</p>
            )}
          </div>
        </div>
        <div className="text-right">
          <span className="block text-body font-semibold text-ink tabular-nums">{mealKcal}</span>
          <span className="text-micro text-ink-3">kcal</span>
        </div>
      </div>
          ...
          return (
            <div key={item.id} className="flex items-center gap-3 border-t border-hairline px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-body font-medium text-ink truncate leading-tight">{name}</p>
                <p className="text-caption text-ink-3 mt-[2px] truncate tabular-nums">
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <button
                  type="button"
                  onClick={() => onEdit(item)}
                  className="flex h-[30px] w-[30px] items-center justify-center rounded-control bg-surface-2 tap-scale"
