...
import {
  X, ScanLine, Camera, Loader2, RefreshCw, CheckCircle2, AlertCircle, Hash, Search,
} from 'lucide-react'
...
export function CameraModal({ onClose, onFoodFound }: Props) {
  ...
  // true = BarcodeDetector API exists in this browser (Android Chrome, desktop Chrome)
  const [barcodeSupport, setBarcodeSupport] = useState(false)
  const [mode, setMode]                     = useState<Mode>('photo') // set properly after mount
  ...
  // Manual barcode entry (used on iOS / browsers without BarcodeDetector)
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        if (!video) return
        // Set attrs imperatively — JSX props alone are unreliable on some mobile browsers
        video.setAttribute('playsinline', 'true')
        video.setAttribute('webkit-playsinline', 'true')
        video.muted = true
        video.srcObject = stream
        // Attempt play immediately; fall back to canplay/loadeddata events
        const tryPlay = () => video.play().catch(() => {})
        video.addEventListener('loadedmetadata', tryPlay, { once: true })
        video.addEventListener('canplay', tryPlay, { once: true })
        // Some devices need an explicit load() after srcObject assignment
        video.load()
      })
      .catch((e: Error) => {
  ...
  useEffect(() => {
    ...
    async function loop() {
      ...
      } catch { /* detector errors on some frames — ignore */ }
      if (active) rafRef.current = requestAnimationFrame(loop)
    }
  ...
  // ── Barcode lookup (shared by auto-detect and manual entry) ─────────────────
  const onBarcodeFound = useCallback(async (code: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${msg} — try searching by name instead.`,
        variant: 'error',
      })
  ...
  const retake = useCallback(() => {
    setCaptured(null); setResults(null); setSelected(null)
  ...
  // ── Tabs: which ones to show ─────────────────────────────────────────────────
  // barcodeSupport → Barcode | Photo | Manual(type)
  // no barcodeSupport → Photo | Manual(type)
  const tabs: { value: Mode; label: string; icon: React.ReactNode }[] = [
    ...(barcodeSupport
      ? [{ value: 'barcode' as Mode, label: 'Barcode', icon: <ScanLine className="h-4 w-4" /> }]
      : []),
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black">
      ...
      {/* Viewport — hidden in manual mode */}
      {mode !== 'manual' && (
      ...
      {/* Manual barcode mode — no camera needed */}
      {mode === 'manual' && (
        <div className="flex-1 flex flex-col items-center justify-center px-8 gap-6">
          ...
          <div className="w-full space-y-3">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 rounded-2xl bg-orange-600 py-3 text-sm font-bold text-white hover:bg-orange-700 active:scale-[.98] disabled:opacity-50 transition-all"
            >
              {manualLoading
      ...
      <div className="shrink-0 bg-gray-950 px-4 pb-6 pt-4 space-y-4">
        ...
        {!results && !analyzing && (
          <>
            {/* Tabs — always shown (barcode tab only on supported browsers) */}
