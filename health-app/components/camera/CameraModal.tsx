        ...
        >
          {camError ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 px-8 text-center">
              ...
              <p className="text-white/80 text-body">{camError}</p>
            </div>
          ) : (
            <>
              {captured ? (
              ...
              {mode === 'barcode' && !captured && (
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <p className="mt-5 text-white/60 text-caption">{barcodeLoading ? 'Looking up product…' : 'Point camera at a barcode'}</p>
                </div>
              )}
              ...
              {mode === 'photo' && analyzing && (
                <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-white font-medium text-body">Identifying food…</p>
                </div>
              )}
            </>
          )}
      ...
      {mode === 'manual' && (
        <div className="flex-1 flex flex-col items-center justify-center px-8 gap-6">
          <div className="text-center">
            ...
            <p className="text-white font-semibold text-body-lg">Enter barcode number</p>
            <p className="text-white/50 text-body mt-1">Type or paste the barcode from any packaged product</p>
          </div>
          <div className="w-full space-y-3">
            <input
              ...
              className="w-full rounded-control bg-white/10 border border-white/20 text-white text-center text-title-sm font-mono px-4 py-3.5 outline-none focus:border-[var(--energy)] placeholder:text-white/30 transition-colors"
              autoFocus
            />
            <Button
          ...
          <p className="text-white/30 text-caption text-center">
          ...
          <div className="space-y-4">
              ...
              <div className="flex gap-2 flex-wrap">
                {results!.map((r, i) => (
                  <button
                    ...
                    className={`flex-shrink-0 rounded-full px-3 py-1 text-caption font-semibold transition-colors tap-scale ${
            ...
            {confidence === 'low' && (
              <div className="flex items-start gap-2 rounded-card bg-energy-soft border border-hairline px-3 py-2.5">
                ...
                <p className="text-caption font-medium leading-snug text-energy-ink">
              ...
              {editingName ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 font-display text-title-sm font-bold text-ink bg-transparent outline-none pb-0.5 border-b-2 border-brand"
                    autoFocus
                  />
                  <button
                    onClick={() => setEditingName(false)}
                    className="text-caption font-bold shrink-0 text-brand-ink"
                  >
                    Done
                  </button>
                </div>
                ...
                >
                  <p className="font-display text-title-sm font-bold text-ink leading-tight">{customName}</p>
                  <Pencil className="h-3.5 w-3.5 text-ink-2 group-hover:text-ink transition-colors shrink-0" />
                </button>
              )}
              {selected!.food.brand && (
                <p className="text-caption text-ink-2 mt-0.5">{selected!.food.brand}</p>
              )}
            </div>
            ...
            <div className="rounded-card bg-energy-soft border border-hairline p-4 space-y-3">
              ...
              <div className="flex items-baseline gap-1.5">
                <span className="font-display text-display font-bold tabular-nums text-ink leading-none">{kcal}</span>
                <span className="text-body font-medium text-energy-ink">kcal</span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t border-hairline">
                {[
                  ...
                  <div key={label} className="flex flex-col gap-0.5">
                    <span className="text-micro font-semibold text-ink-2">{label}</span>
                    <span className="text-body font-bold tabular-nums" style={{ color }}>
                      {value}<span className="text-micro font-medium text-ink-2">g</span>
                    </span>
                  </div>
                ))}
              </div>
            ...
            {coaching && (
              <p className="px-1 text-caption leading-relaxed text-ink-2">💡 {coaching}</p>
            )}

            {/* Portion: number input + slider */}
            <div>
              <p className="text-caption text-ink-2 mb-2">Portion size</p>
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1 shrink-0">
                  <input
                    ...
                    className="w-[64px] text-center text-body font-bold text-ink rounded-control py-1.5 outline-none bg-surface-2 border border-hairline"
                  />
                  <span className="text-caption text-ink-2 font-medium">{selected?.unit ?? 'g'}</span>
                </div>
                <input
            ...
            <div className="flex gap-2">
              <select
                ...
                className="flex-1 rounded-control text-body py-2.5 px-3 outline-none transition-colors bg-surface-2 border border-hairline text-ink"
              >
                {MEAL_OPTIONS.map((o) => (
            ...
            <button
              onClick={retake}
              className="flex w-full items-center justify-center gap-1.5 py-1 text-caption font-medium text-ink-2 hover:text-ink transition-colors"
            >
          ...
          <div className="space-y-3">
            {showContextInput ? (
              <input
                ...
                maxLength={200}
                className="w-full rounded-control bg-white/10 border border-white/20 text-white text-body px-4 py-3 outline-none focus:border-[var(--energy)] placeholder:text-white/40 transition-colors"
                autoFocus
              />
            ) : (
              <button
                onClick={() => setShowContextInput(true)}
                className="flex items-center gap-1.5 text-caption font-medium text-white/50 hover:text-white transition-colors"
              >
            ...
            <div className="flex gap-2">
              <button
                onClick={retake}
                className="flex items-center justify-center gap-1.5 rounded-control px-4 h-12 text-body font-semibold text-white/60 hover:text-white bg-white/10 transition-colors tap-scale"
              >
            ...
            <div className="flex rounded-control bg-white/10 p-1 gap-0.5">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 rounded-control py-2 text-caption font-semibold transition-colors ${
            ...
            {mode === 'photo' && (
              <div className="flex items-center justify-center gap-6 pt-1">
                ...
                >
                  ...
                  <span className="text-micro font-medium text-white/40 group-hover:text-white/60 transition-colors">Gallery</span>
                </button>
