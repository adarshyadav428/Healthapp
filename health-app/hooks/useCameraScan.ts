...
export function useCameraScan({ onClose, onFoodFound, logDate, context = 'standalone' }: Params) {
  ...
  // One key per analysed photo, set fresh whenever a new result set lands and
  // reused across every retry of logFood for THAT result (a failed log leaves
  // `results` in place) — never regenerated by a retry, only by a genuinely
  // new analyse (retake clears `results`, and the next analyzePhoto success
  // sets a fresh id here before logFood can run again). See
    ...
    try {
      ...
      const res = multi
        ...
        : await fetch('/api/logs/add', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', ...logMetaHeaders('photo_scan') },
            body: JSON.stringify({ food_id: selected.food.id, meal, servings: 1, grams, date: logDate, client_request_id: clientRequestIdRef.current }),
          })
