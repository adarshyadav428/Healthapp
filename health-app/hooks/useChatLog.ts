...
export function useChatLog({ onClose, logDate, context = 'standalone' }: Params) {
  ...
  // One key per analysed message, set fresh whenever a new 'confirm' screen is
  // reached and reused across every retry of THAT screen (a failed log leaves
  // state back at 'confirm' with the same items) — never regenerated by a
  // retry, only by a genuinely new analyse. See insertIdempotentBatch.
  const clientRequestIdRef = useRef<string | null>(null)
  // Which model produced the current items — reported by the route and
  // attached to the correction event so accuracy can be compared across models.
  const scanModelRef = useRef<string | null>(null)
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      scanModelRef.current = typeof data.model === 'string' ? data.model : null
