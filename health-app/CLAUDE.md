...
- **Chrome:** `components/layout/AppHeader.tsx` (wordmark/`title` + theme toggle + avatar) is used on **secondary** pages only; the four core tab screens use per-page headers instead (see the Ember Air note above). `BottomNav` is the shared tab bar + FAB on every authenticated page.
