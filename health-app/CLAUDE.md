...
- **Chrome:** one header, `components/layout/AppHeader.tsx` — always shows the wordmark or a `title` prop plus the theme toggle and avatar. Page-specific greetings/headings live in the page body below it, not in the header. `BottomNav` is the tab bar + FAB.
