...
- **INR pricing only** — never USD. Prices: Pro Monthly ₹299, Pro Annual ₹1,999 (changed 2026-07-19 from ₹199/₹699). Both plans carry a **3-day** free trial — Play Console offers only (no Razorpay/web trial), so trial copy renders only inside the TWA.
- **Free tier** — unlimited manual/search food logs, barcode scanning, exercise logging, weight tracking, and 7 days of history. **AI features (camera photo scan and chat meal logging) are Pro-only, apart from a lifetime free trial of `AI_TRIAL_SCANS` (3) calls that unlocks only once the user's email is verified** — changed 2026-07-18, previously 5 scans/day and 10 chat logs/day were free. Both are server-enforced with a 403 in `app/api/camera/analyze/route.ts` / `app/api/chat/analyze/route.ts`; the UI is not the boundary. Rules live in `lib/aiTrial.ts` (pure, importable by client code) and `lib/aiTrialServer.ts` (the DB reads). Rationale: every AI call is a paid Gemini request, and with email confirmation disabled at signup an account costs an abuser nothing to create — so the allowance is *lifetime* (cost scales with signups, not with signups × days) and *gated on a verified inbox* (so it can't be farmed by re-registering). The trial exists because the AI scan is the product's wow moment and a paywall in front of an unexperienced feature converts badly. Camera and chat share ONE pool of 3, counted as rows in `camera_photo_logs` + `chat_logs` (append-only, so a row count is the lifetime figure — no extra column). Both counts fail CLOSED. Pro also unlocks history beyond 7 days and custom foods. **If you change this, update the free-tier bullets on `app/page.tsx` too** — that list is a public claim.
