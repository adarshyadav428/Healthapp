    ...
    <div className="min-h-screen bg-canvas pb-24">
      ...
      <main className="mx-auto w-full max-w-md px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="rounded-card bg-surface p-3 shadow-rest border border-hairline h-20 animate-shimmer" />
          ))}
        </div>

        {/* Chart */}
        <div className="rounded-sheet bg-surface p-4 shadow-rest border border-hairline">
        ...
        <div className="space-y-2">
          ...
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-14 w-full rounded-card bg-surface shadow-rest border border-hairline animate-shimmer" />
          ))}
        </div>
      </main>
    </div>
