    ...
    <div className="min-h-screen overflow-x-hidden">
      ...
      <header className="mx-auto flex w-full max-w-4xl items-center justify-between px-5 py-5">
        ...
        <div className="flex items-center gap-3">
          <Link href="/auth/sign-in" className="text-body font-medium text-ink-2 hover:text-ink transition-colors">
            Sign in
          </Link>
          <Link
            href="/auth/sign-up"
      ...
      <main className="mx-auto w-full max-w-4xl px-5 pb-24">
        ...
        <section className="py-10 text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-hairline bg-surface px-3 py-1 text-caption font-semibold text-ink-2 mb-6">
            🇮🇳 Built for Indian diets &middot; 850+ desi foods included
          </div>
          <h1 className="font-display text-display font-bold leading-tight text-ink sm:text-hero">
          ...
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <Link
              href="/auth/sign-up"
              className="rounded-full bg-brand px-8 py-3 text-body-lg font-bold text-white hover:opacity-90 transition-opacity shadow-float"
            >
              Start for free →
            </Link>
            <Link
              href="/auth/sign-in"
              className="rounded-full border border-hairline bg-surface px-8 py-3 text-body-lg font-semibold text-ink hover:bg-surface-2 transition-colors"
            >
              Sign in
            </Link>
          </div>
          <p className="mt-3 text-caption text-ink-2">Free forever · No credit card · Installs like an app</p>
        </section>
        ...
        <section className="mx-auto max-w-sm">
          <div className="rounded-sheet border border-hairline bg-surface p-5 shadow-float space-y-3">
            ...
            <div className="rounded-card bg-surface border border-hairline p-4 flex items-center justify-between">
              <div>
                <p className="text-caption text-ink-2 font-semibold uppercase tracking-caps">Calories today</p>
                <p className="font-display text-title-lg font-bold text-ink tabular-nums">1,420</p>
                <p className="text-body text-ink-2">of 1,800 goal</p>
                <p className="text-body font-bold text-good mt-1">380 kcal remaining</p>
              </div>
              <div className="relative h-24 w-24 flex-shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-body-lg font-bold text-energy-ink tabular-nums">380</span>
                  <span className="text-micro text-ink-2">left</span>
                </div>
              </div>
            </div>
            {/* Macro cards mock */}
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div key={m.label} className="rounded-control bg-surface-2 p-2">
                  <p className="text-micro font-semibold text-ink-2 uppercase">{m.label}</p>
                  <p className="text-body font-bold text-ink tabular-nums">{m.val}</p>
                  <p className="text-micro text-ink-2">{m.target}</p>
            ...
            <div className="rounded-card bg-surface-2 p-3 space-y-2">
              <p className="text-caption font-bold text-ink-2 flex items-center gap-1">
                🥣 Breakfast
                <span className="ml-auto text-caption font-semibold text-ink">486 kcal</span>
              </p>
              {[
                ...
                <div key={f.name} className="flex justify-between text-caption bg-surface rounded-control px-3 py-2">
        ...
        <section className="mt-16 grid gap-4 sm:grid-cols-2">
          {[
            ...
            <div key={f.title} className="rounded-sheet border border-hairline bg-surface p-5 shadow-rest">
              <span className="text-title">{f.emoji}</span>
              <h3 className="mt-2 text-body-lg font-bold text-ink">{f.title}</h3>
              <p className="mt-1 text-body text-ink-2">{f.desc}</p>
            </div>
          ))}
        </section>
        ...
        <section className="mt-16">
          <div className="rounded-sheet border border-hairline bg-surface bg-hero-wash p-6 shadow-rest relative overflow-hidden">
            <div className="relative">
              <div className="inline-flex items-center gap-2 rounded-full border border-hairline bg-brand-soft px-3 py-1 text-caption font-semibold text-brand-ink mb-4">
                👤 Why I built this
              </div>
              <blockquote className="font-display text-title-sm font-bold text-ink leading-snug mb-4">
              ...
              <p className="text-body text-ink-2 leading-relaxed mb-5">
              ...
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-full bg-brand flex items-center justify-center text-white font-bold text-body shrink-0">
                  A
                </div>
                <div>
                  <p className="text-body font-bold text-ink">Adarsh Yadav</p>
                  <p className="text-caption text-ink-2">Founder · Azamgarh, UP 🇮🇳</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="mt-16">
          <h2 className="font-display text-title font-bold text-ink text-center mb-8">Simple, honest pricing</h2>
          <div className="grid gap-4 sm:grid-cols-2">
            {/* Free */}
            <div className="rounded-sheet border border-hairline bg-surface p-6 shadow-rest">
              <p className="text-caption font-semibold uppercase tracking-caps text-ink-2 mb-1">Free forever</p>
              <p className="font-display text-title-lg font-bold text-ink">₹0</p>
              <p className="text-body text-ink-2 mt-1 mb-5">No credit card required</p>
              <ul className="space-y-2.5">
                {[
                  ...
                  <li key={item} className="flex items-center gap-2 text-body text-ink-2">
              ...
              <Link
                href="/auth/sign-up"
                className="mt-6 block w-full rounded-control border border-hairline bg-canvas py-3 text-body font-bold text-ink text-center hover:bg-surface-2 transition-colors"
              >
                Start for free
              </Link>
            </div>
            ...
            <div className="rounded-sheet border-2 border-brand bg-brand-soft p-6 shadow-float relative overflow-hidden">
              <div className="absolute top-4 right-4 rounded-full bg-energy px-2.5 py-1 text-micro font-bold text-energy-ink uppercase tracking-caps">Popular</div>
              <p className="text-caption font-semibold uppercase tracking-caps text-brand-ink mb-1">Pro</p>
              <p className="font-display text-title-lg font-bold text-ink">₹299<span className="text-body-lg font-semibold text-ink-2">/mo</span></p>
              <p className="text-body text-ink-2 mt-1 mb-5">or ₹1,999/year · save 44%</p>
              <ul className="space-y-2.5">
                {[
                  'Everything in Free',
              ...
              <Link
                href="/upgrade"
                className="mt-6 block w-full rounded-control bg-brand py-3 text-body font-bold text-white text-center hover:opacity-90 transition-opacity shadow-rest"
              >
                Upgrade to Pro →
              </Link>
            </div>
        ...
        {/* FAQ */}
        <section className="mt-16">
          <h2 className="font-display text-title font-bold text-ink text-center mb-8">Frequently asked questions</h2>
          <div className="space-y-3">
            {[
              ...
              <details key={faq.q} className="group rounded-card border border-hairline bg-surface px-5 py-4 shadow-rest cursor-pointer">
                <summary className="list-none flex items-center justify-between font-semibold text-body text-ink">
                  {faq.q}
                  <span className="text-ink-2 text-title-sm leading-none group-open:rotate-45 transition-transform">+</span>
                </summary>
                <p className="mt-3 text-body text-ink-2 leading-relaxed">{faq.a}</p>
              </details>
            ))}
          </div>
        </section>
        ...
        <section className="mt-16 rounded-sheet border border-hairline bg-surface p-8 text-center shadow-rest">
          <h2 className="font-display text-title font-bold text-ink">Ready to start?</h2>
          <p className="mt-2 text-body text-ink-2">Join GetInShape and take control of your nutrition — the Indian way.</p>
          <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <Link
              href="/auth/sign-up"
              className="rounded-full bg-brand px-8 py-3 text-body font-bold text-white hover:opacity-90 transition-opacity shadow-rest"
            >
              Start for free →
            </Link>
          </div>
          <p className="mt-3 text-caption text-ink-2">No credit card · Cancel anytime</p>
        </section>
      </main>
