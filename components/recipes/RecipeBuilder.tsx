...
export function RecipeBuilder() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-3xl border border-orange-100 bg-white/90 p-4 shadow-sm space-y-3">
        <div>
          <label className="block text-xs font-semibold uppercase tracking-wide text-gray-400 mb-1">Recipe name</label>
          <input
            value={recipeName}
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="h-9 w-9 rounded-full bg-gray-100 text-lg font-bold text-gray-600 hover:bg-gray-200 active:scale-90 transition-all"
            >
              −
            </button>
            <span className="text-2xl font-black text-gray-900 w-8 text-center">{servings}</span>
            <button
              type="button"
      ...
      <div className="rounded-3xl border border-gray-100 bg-white/90 p-4 shadow-sm space-y-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-9 h-10 text-sm rounded-2xl border border-gray-200 bg-gray-50 outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all"
          />
          {query && (
        ...
        {searchOpen && query.length > 1 && (
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {searching ? (
              ...
              <p className="text-xs text-gray-400 text-center py-3">No results — try another search</p>
            ) : (
              (searchResults ?? []).map((food) => (
                <button
                  ...
                  className="w-full flex items-center justify-between rounded-xl border border-gray-100 bg-white px-3 py-2 text-left hover:border-orange-200 hover:bg-orange-50 transition-all group"
                >
                  <div className="min-w-0 flex-1 mr-2">
                    <p className="text-sm font-semibold text-gray-800 truncate">{food.name}</p>
                    <p className="text-[11px] text-gray-400">{Math.round(food.kcal_per_100g)} kcal · {Math.round(food.protein_g_per_100g)}P {Math.round(food.carbs_g_per_100g)}C {Math.round(food.fat_g_per_100g)}F per 100g</p>
                  </div>
        ...
        {ingredients.length > 0 ? (
            ...
            {ingredients.map((ing, index) => (
              <div key={ing.food.id} className="flex items-center gap-2 rounded-2xl border border-gray-100 bg-gray-50 px-3 py-2">
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-gray-800 truncate">{ing.food.name}</p>
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    className="w-16 text-center text-xs font-bold rounded-xl border border-gray-200 bg-white px-2 py-1.5 outline-none focus:border-orange-400"
                  />
                  <span className="text-[10px] text-gray-400">g</span>
                  <button
                    type="button"
                    onClick={() => removeIngredient(index)}
                    className="rounded-full p-1 text-gray-300 hover:text-rose-500 hover:bg-rose-50 transition-colors"
                  >
          ...
          <div className="text-center py-4">
            ...
            <p className="text-xs text-gray-400">Search and add ingredients above</p>
          </div>
        )}
      </div>
