...
export function ExerciseLogger({ weightKg = 70 }: Props) {
  ...
  return (
    <div className="rounded-3xl border border-gray-100 dark:border-slate-700 bg-white/90 dark:bg-slate-900/80 shadow-sm overflow-hidden">
      {/* Header toggle */}
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {totalCaloriesBurned > 0 && (
            <span className="rounded-full bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 text-xs font-bold px-2 py-0.5">
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-gray-50 dark:border-slate-800">
          {/* Today's exercise logs */}
          {logs.length > 0 && (
            <div className="space-y-1.5 pt-3">
              {logs.map(log => (
                <div key={log.id} className="flex items-center justify-between rounded-2xl bg-rose-50 dark:bg-rose-950/20 px-3 py-2">
                  <div>
                  ...
                  <button
                    ...
                    className="rounded-full p-1 text-muted hover:text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-950/40 transition-colors"
                  >
          ...
          <div>
              ...
              {COMMON_ACTIVITIES.map(a => (
                <button
                  ...
                  className={`rounded-xl px-2.5 py-1 text-xs font-semibold border transition-all ${
                    activity === a.name
                      ? 'bg-rose-100 dark:bg-rose-950/50 text-rose-700 dark:text-rose-300 border-rose-300 dark:border-rose-700'
                      : 'bg-gray-50 dark:bg-slate-800 text-muted border-gray-100 dark:border-slate-700 hover:border-rose-200'
                  }`}
                >
                  {a.name}
                </button>
            ...
            <input
              ...
              className="mt-2 w-full rounded-2xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-3 py-2 text-sm text-foreground outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100 dark:focus:ring-rose-900 transition-all placeholder:text-muted"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                min={1}
                className="w-full rounded-2xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-3 py-2 text-sm font-bold text-center text-foreground outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100 dark:focus:ring-rose-900 transition-all"
              />
            </div>
            <div>
